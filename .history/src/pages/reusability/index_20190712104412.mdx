...
One thing to note is that under normal circumstances, when we call our `addToN` function and pass it an argument, say `5`, the function creates a local (to the function) variable called `n` and assigns it the value `5`. We can then use it inside our `addToN` function.
