...
Suppose we want to add up numbers from an array to get the sum. We could use a loop, but that's so last millennium. Let's try another approach using functions **without loops**. We'll see more of this in the next section, conveniently titled <Link to="/iteration/" aria-label="Iteration"><em>Loops are so last millennium!</em></Link>
...
You might say an `Error` object, and in some instances that might be the best option. But in functional programming, we try to avoid using errors wherever possible. And error in functional programming should mean _the damn thing is broke_ and there ain't no fixing it.
