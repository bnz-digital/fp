...
Suppose we want to add up numbers from an array to get the sum. We could use a loop, but that's so last millennium.

Let's try another approach using functions _without loops_. We'll see more of this in the next section, conveniently titled, <Link to="/iteration/" aria-label="Iteration"><em>Loops are so last millennium!</em></Link>
