...
export const addTodo = name => cy.get('.new-todo').type(\`\${name}{enter}\`)\n
export const getTopTodoFromList = () => cy.get('.todo-list').eq(0)\n
export const getTodoLabel = todo => todo.find('label').text()`} />
