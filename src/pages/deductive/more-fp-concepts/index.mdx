...
We can also use libraries of true FP functions to do the same thing, but without binding our function to a specific array instance. Compare, for example, the [Ramda](https://ramdajs.com/docs/#map) `map` function to the built-in, vanilla JS `Array.map` _method_.
...
We can use [Ramda](https://ramdajs.com/docs/) to enable us to easily write functional code:
...
What we want is a function that let's us apply the arguments in any combination. [Ramda](https://ramdajs.com/docs/#curry) provides us with just such a function:
